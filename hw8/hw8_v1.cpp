...
inline int minimalcover(int& cases, int& circles){
    ...
    int num_of_zero = 0;
    for (int i = 0; i < circles; ++i){
        double x = sqrt(4 * rad_pos[cases][i].first * rad_pos[cases][i].first - w[cases] * w[cases])/2;
        if (x<0 || isnan(x) || 2*rad_pos[cases][i].first <= w[cases]){
            x=0;
            x_left_rad_pos[i] = make_pair(0, make_pair(0, make_pair(0, 0)));
            num_of_zero + 1;
            continue;
        }
        x_left_rad_pos[i] = make_pair(rad_pos[cases][i].second-x,make_pair(x, rad_pos[cases][i]));
    }
